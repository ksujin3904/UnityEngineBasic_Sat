...
namespace Example05_MyHashtable
{
    internal class MyHashtable<T, K>
    {
        ...
        {
            _tmpHash = Hash(key);
            if (_bucket[_tmpHash] == null) // 배열이 존재하지 않는 경우
        ...
        public bool ContainsKey(T target)
        {
            ...
            if(_bucket[_tmpHash] != null && _bucket[_tmpHash].Count>0)
                return true;
            return false;
        }

        // 2.
        public bool TryGetValue(T key, out K value) // out: bool 뿐만 아니라 추가로 value값을 반환하게 도와줌
        {
            bool isOK = true;
            value = default(K);
            _tmpHash = Hash(key);
        ...
        {
            _tmpHash = 0;
            string tmpString = key.ToString(); // T타입을 박싱하여 문자열 변환
